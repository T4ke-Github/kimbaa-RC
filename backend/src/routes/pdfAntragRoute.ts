...
export const pdfAntragRouter = express.Router();


pdfAntragRouter.get('/:studentId', requiresAuthentication, async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        try {
            ...
            const [antragPDFBuffer] = await createAntragPDF(user.id!.toString());
            ...
            res.setHeader('Content-Disposition', `attachment; filename="antrag.pdf"`);
            res.setHeader('Content-Length', antragPDFBuffer.length);
    
            
            res.status(200).send(antragPDFBuffer);
        } catch (pdfError) {
